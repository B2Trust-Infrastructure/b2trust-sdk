...
try {
  ...
  console.log(`Found ${results.meta.total} results for "${results.query}" (${results.meta.query_time_ms}ms)\n`);
  if (results.data.length === 0) {
    console.log('No companies found.');
  } else {
    console.table(results.data.slice(0, 20).map((c) => ({
      Country: c.country_code,
      Name: c.company_name,
      ID: c.national_id,
      Status: c.status,
      Sources: c.registry_count,
